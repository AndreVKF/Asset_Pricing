...
class MainView(Generic):
    ...
    def calculateFRAs(self, requestDate, Instrument):
        # Get ViewName/CalcType
        Instrument_Row = self.DF_Instruments[(self.DF_Instruments['Name']==Instrument)]

        # Check if instrument calculates FRA
        if not Instrument_Row['CalculateFRA'].values[0]:
            print("Instrument not for FRA calculation.")
            return -1
        ...
        # Get Value Type
        CalcValueType = self.DF_ValueType[(self.DF_ValueType["Id"]==Instrument_Row['Id_ValueType'].values[0])]['Name'].values[0]
        ...
        if Id_CalcType == 1:
            FRA_DF['Base_Days'] = FRA_DF.apply(lambda x: self.cal.bizdays(x.Refdate.strftime("%Y-%m-%d"), x.Base_Valuation.strftime("%Y-%m-%d")), axis=1)
            FRA_DF['End_Days'] = FRA_DF.apply(lambda x: self.cal.bizdays(x.Refdate.strftime("%Y-%m-%d"), x.End_Valuation.strftime("%Y-%m-%d")), axis=1)
            FRA_DF['Between_Days'] = FRA_DF.apply(lambda x: self.cal.bizdays(x.Base_Valuation.strftime("%Y-%m-%d"), x.End_Valuation.strftime("%Y-%m-%d")), axis=1)
        ...
        # Base Days 360
            ...
            FRA_DF['Base_Days'] = FRA_DF.apply(lambda x: self.days360(x.Refdate, x.Base_Valuation), axis=1)
            FRA_DF['End_Days'] = FRA_DF.apply(lambda x: self.days360(x.Refdate, x.End_Valuation), axis=1)
            FRA_DF['Between_Days'] = FRA_DF.apply(lambda x: self.days360(x.Base_Valuation, x.End_Valuation), axis=1)
        ...
        return FRA_DF[self.AP_Connection.getData(query=self.Queries.columnNamesFromTable(tableName='FRA_Tb'))['COLUMN_NAME'].to_list()]
